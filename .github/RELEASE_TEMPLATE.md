...
- 🧭 **Compass** with a 360° dial, smooth wrap-around heading, adaptive magnetic-interference detection and in-app magnetometer calibration (hard/soft iron). **Fixed a 90° compass-rose error**: the dial ticks, cardinal labels and the Qibla Kaaba marker are now drawn on the true physical compass rose (0° = top, clockwise), so the marker agrees with the turn hints — a Qibla on your right renders on the right, and turning toward it brings it to the top instead of away. The rose is layout-direction independent, rendering identically in **LTR and RTL** (a compass must not mirror); pinned by 8 new `CompassRoseTest` unit tests. Both degree readouts — the heading above the dial and the Qibla card's bearing — now render at **headlineLarge (32 sp)** in **fixed-width slots**, uniform with the level screen's readouts: smaller, so no number overshadows the dial, and pinned, so the digits never shift when the value changes (location update, north-reference switch). Three professional dial styles (**Classic / Azimuth / Minimal**) switchable from Settings, with granular per-element toggles (cardinal labels, degree ticks, degree numbers, heading readout, north-reference badge, source/declination details, accuracy panel) and an **auto-hide** option that hides the numbers whenever the magnetic accuracy is low. All readouts and bottom details sit in **closed cards with fixed-width numeric slots**, so the digits changing as you move the phone never shift the text.
...
- 📏 **Level** — automatically switches by how the device is held: flat shows a two-axis bubble (four directions); held upright shows a water/mercury **tube level** (left-right) with a **plumb gauge** whose needle shows the deviation-from-vertical on a degree scale. Every moving element (mercury bubble, plumb needle, flat bubble) **glides smoothly** instead of snapping, and reaching perfect level lights a **pulsing centered dot** in both modes. The gauge conventions are **pinned in the domain layer** (pure functions with the correct convention documented — 0° = straight down, positive = right, top-up hemisphere = negative deviation; the geometry renders identically in **LTR and RTL**, since a level must not mirror) and covered by 7 new regression tests. Feedback on center: a **graded haptic** that ramps up in strength as you approach level/plumb in **both** modes (proximity bands 8°→0.5°), plus a soft **confirmation chime** with its own settings toggle, independent of the haptics toggle. Zero-point calibration.
...
- 202 unit tests (math, geodesy, geomagnetic WMM2025 model, solar position, engines, calibration, robustness, gauge/bubble conventions), lint-clean; instrumented UI tests run on a device — including `CompassDialQiblaMarkerTest`, which asserts the Kaaba marker's pixel position matches the turn hint's sign in both LTR and RTL.
...
- Instrumented UI tests require a connected device/emulator (`./gradlew :presentation:connectedDebugAndroidTest` for the compass/level module, `./gradlew connectedDebugAndroidTest` for the app).
